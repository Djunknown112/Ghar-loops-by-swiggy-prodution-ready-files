...
export default function AuthScreen({ onAuthSuccess, onContinueAsGuest }: AuthScreenProps) {
  ...
  // On mount, check if the browser is returning from a Google redirect-based
  // sign-in (this fires when signInWithPopup was blocked/failed and we fell
  // back to signInWithRedirect below).
  useEffect(() => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Popups are blocked or broken in many production/embedded contexts
      // (strict browser popup blockers, COOP policies, in-app browsers).
      // In those specific cases, fall back to a full-page redirect flow,
      // which works even where popups are silently blocked.
      ...
      if (popupFailureCodes.includes(err.code)) {
        try {
          ...
          return; // page will navigate away; result is handled on return via getRedirectResult
        } catch (redirectErr: any) {
          console.error("Google sign-in (redirect) error:", redirectErr.code, redirectErr.message);
          setError("Google sign-in isn't working in this browser. Try a different browser or continue as guest.");
          setLoading(false);
          return;
        }
      }

      // Specific, actionable messages for the most common real failure causes.
      if (err.code === "auth/unauthorized-domain") {
        setError("This site's domain isn't authorized for Google sign-in yet. (Admin: add this domain under Firebase Console → Authentication → Settings → Authorized domains.)");
