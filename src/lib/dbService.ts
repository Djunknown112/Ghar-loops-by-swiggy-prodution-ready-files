...
export const dbService = {
  ...
  async signInWithGoogle(): Promise<UserProfile> {
    try {
      ...
      if (userSnap.exists()) {
        ...
        // Create new user profile, pre-populating with default budget and a smart default persona
        profile = {
          uid: user.uid,
      ...
      console.error("Firebase Sign-In failed, falling back to mock guest login:", error);
      // Fallback guest login so iframe demo never fails to interact
      return {
        uid: "guest-user",
        displayName: "Dheeraj Joshi (Guest)",
        email: "dheeraj.guest@gmail.com",
        monthlyBudget: 5500,
        persona: "Family Planner"
      };
    }
  ...
  async getSchedules(isGuest: boolean): Promise<ScheduleItem[]> {
    ...
    try {
      ...
      if (items.length === 0) {
        // Seed initial local schedule to Firestore if empty
        for (const item of INITIAL_SCHEDULE) {
          await setDoc(doc(db, "users", uid, "schedule", item.id), item);
          items.push(item);
        }
      }
      return items;
  ...
  async getOrderHistory(isGuest: boolean): Promise<OrderRecord[]> {
    ...
    try {
      ...
      if (items.length === 0) {
        for (const item of INITIAL_ORDER_HISTORY) {
          await setDoc(doc(db, "users", uid, "orderHistory", item.id), item);
          items.push(item);
        }
      }
      ...
      return getLocalStorage<OrderRecord[]>("gharloop_orders", INITIAL_ORDER_HISTORY).sort((a, b) => b.date.localeCompare(a.date));
    }
  },
  ...
  async getApprovals(isGuest: boolean): Promise<ApprovalRequest[]> {
    ...
    try {
      ...
      if (items.length === 0) {
        for (const item of INITIAL_APPROVALS) {
          await setDoc(doc(db, "users", uid, "approvals", item.id), item);
          items.push(item);
        }
      }
      return items;
  ...
  async getNotifications(isGuest: boolean): Promise<AppNotification[]> {
    ...
    try {
      ...
      return getLocalStorage<AppNotification[]>("gharloop_notifications", INITIAL_NOTIFICATIONS).sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    }
  },
